...
// Destructor, cleans up window.
Element::~Element()
{
    if (window_) terminal().remove_window(window_);
    window_ = nullptr;
}
...
// Returns a pointer to the attached Window element. Use with caution!
Window* Element::window_ptr() { return window_; }
